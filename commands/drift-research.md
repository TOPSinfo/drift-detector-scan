...
3. **Fetch the vendor's own page — structured source FIRST, in this order** (cheapest + most reliable first; the date must come from the VENDOR'S page, never a blog or a search snippet):
   ...
   - **whenever a direct fetch fails to yield readable text**, retry through a hosted render-to-markdown reader by prefixing the URL: `WebFetch https://r.jina.ai/<the-full-url>`. TWO distinct failure modes both recover this way, and you must try it for BOTH:
     - a **JS-only SPA** that returns an empty/shell body (this recovered Amazon Ads' deprecations SPA that a plain fetch could not);
     - a **bot-wall / WAF refusal** — HTTP **403**, a Cloudflare interstitial, "Just a moment…". The reader defeats these too. Retrying the same URL with `curl` and a browser User-Agent does **not** help and is not worth the attempt; the whole of `developer.sage.com` refused both `WebFetch` and a UA-spoofed `curl` with 403, then returned clean markdown through the reader on the first try (2026-08-25, which is how Sage's dated V3 deprecation was finally read).
     A 403 is therefore **never** grounds for `unverified` until the reader has also failed. It sends only the PUBLIC docs URL to a third party — fine for vendor docs, **never for anything private**. No local dependency, no browser.
     Provenance note: the reader is a proxy, so the bytes reach you second-hand. Cite the **vendor's own URL** as `source_url` (not the `r.jina.ai/…` wrapper) and say in `evidence` that it was read through the reader — so a later human re-checking the entry knows why their own browser may be needed.
4. Decide the verdict:
   ...
   - **`unverified`** — none of the sources in step 3 yielded the vendor's own dated page (a login-gated developer portal, a SPA even the reader can't render, a 403 the reader ALSO could not get past, all URLs 404). Do **not** guess — a blog or a search summary is **not** a source. (UPS's June-2024 access-key cutoff is all over third-party blogs, but its own developer portal won't render even through the reader, so the honest verdict is `unverified`, not a borrowed date.)
