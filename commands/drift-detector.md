...
# The scanner is ALWAYS the engine this plugin ships: bin/drift-scan self-bootstraps its own venv
# (from requirements-plugin.txt) and fetches the pinned ast-grep binary on first run. This guarantees
# engine == orchestration == verify — no version skew, no PyPI/uvx package, nothing else to install.
# If the bundled engine can't be found, fail loudly (exit 4) rather than run anything else.
...
[ -n "$SCAN" ] && [ -x "$SCAN" ] || { echo "drift-detector: no runner — the plugin's bin/drift-scan is missing; reinstall the plugin. It needs uv (https://docs.astral.sh/uv/) or python3.11+ with venv; run the plugin's bin/drift-scan doctor for a full check." >&2; exit 4; }
```
